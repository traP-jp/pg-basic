...
プログラミングにおいてはこれと少し似ているが、ある値を与えた時に、**決められた処理をして**、ある値を返すものを**関数**と呼ぶ。ここで、関数に値を与える必要はないし、また値を返さなくても良いという事に注意すること。
...
関数を実装するとき、関数の末尾に必ず`return 0;`と記述する（これについては[7.3](3)で扱う）。
