...
::: tip
`Ctrl`+`,` で設定が開かない場合は、タブバーの `∨` ボタンをクリックして「設定」を選んでください。
:::
...
::: tip
拡張機能が表示されない・インストールできない場合は、ウィンドウ左下に「制限モード」などと表示されていないかを確認してください。もし表示されている場合は「管理」をクリックし、「信頼する」を選ぶと解除できます。
:::
...
:::warning TA向け
`settings.json` の中身がほぼ空（`{}`のみ）でも問題ありません。そのままStep 5〜6の内容を追加すれば動作します。
:::
