...
`int x = 4` で `x` という名前の **`int`型の変数** を定義できる。変数を簡単に説明すると、「決められた情報を持つもの」である。
...
:::danger
`C++`では、「型」が異なる値と変数について、値をそのまま変数に代入することはできない。例えば、`int`型の変数に文字（=`string`型とよばれている）を入れるような命令はエラーとなる。
:::
